...
def make_window(cluster_id: str, values: list[tuple[float, float, float]], start=None) -> TelemetryWindow:
    start = start or datetime(2026, 8, 31, 2, 0, tzinfo=timezone.utc)
    readings = [
        Reading(
    ...
    return TelemetryWindow(sensor_cluster_id=cluster_id, network_metadata=NetworkMetadata(), readings=readings)
...
def baseline_store() -> BaselineStore:
    store = BaselineStore()
    store.seed_baseline("cluster-a", BaselineStats(45.0, 0.5, 80.0, 0.5))
    store.seed_baseline("cluster-b", BaselineStats(45.0, 0.5, 80.0, 0.5))
    return store
