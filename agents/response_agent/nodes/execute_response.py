    ...
    def execute_response_node(state: ActuationState) -> ActuationState:
        ...
        if decision == ActuationDecision.LOG_ONLY:
            ...
            try:
                ...
                notification_client.send(recipient=operator_contact,message=f"EMERGENCY: Valve isolation for incident {incident_id} FAILED to confirm. Manual intervention required.",channel="sms", incident_id=incident_id,)
        else:
