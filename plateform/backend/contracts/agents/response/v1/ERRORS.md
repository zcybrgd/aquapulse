...
| `agent_result_duplicate` | `result_id` already stored |
| `agent_identity_unmapped` | External ID has no mapping |
| `agent_result_rejected` | Result rejected; transaction rolled back |
| `physical_commands_disabled` | Valve isolate is blocked |
| `notifications_disabled` | Notify returns `sent: false` |
| `camara_disabled` | QoD reservation is mock/denied |
